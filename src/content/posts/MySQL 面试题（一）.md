...
最后，如果慢查询正在执行，已经导致数据库负载偏高了，但由于慢查询还没执行完，因此慢查询日志还看不到相关语句。这种情况下要快速定位慢查询，可以通过 show processlist 命令查看当前正在运行的线程，输出中 Time 表示已经运行了多久，Info 表示具体的 SQL 语句内容。
...
- 覆盖索引
  ...
  这样的主键索引树称为 _聚簇索引_，而其他唯一索引树称为 _二级索引_。
